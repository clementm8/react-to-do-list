...
function Anchor({ children, href = '/demo', className }) {
    return (
        <Link href={href} className={cn('text-inherit', className)}>{children}</Link>
    )
}
...
function AnchorLabel({ children, className }) {
    return (
        <span className={cn('text-blue-500 font-semibold',className)}>
            {children}
        </span>
    )
}
